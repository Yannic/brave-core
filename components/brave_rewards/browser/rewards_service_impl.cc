...
bool DeleteFilesOnFileTaskRunner(
    base::WeakPtr<WeakWrapperFile> diagnostic_log,
...
bool DeleteDiagnosticLogOnFileTaskRunner(
  ...
  // Must close diagnostic log before deleting it
  if (diagnostic_log) {
    diagnostic_log->Close();
  }
...
bool MaybeTailDiagnosticLog(base::WeakPtr<WeakWrapperFile> diagnostic_log,
                            const int num_lines) {
  if (!diagnostic_log || !diagnostic_log->IsValid()) {
    return false;
  }

  // We do not tail the log on first run so that if the browser
...
RewardsServiceImpl::~RewardsServiceImpl() {
  ...
  if (diagnostic_log_.IsValid()) {
    file_task_runner_->PostTask(FROM_HERE,
                                base::BindOnce([](WeakWrapperFile&& file) {},
                                               std::move(diagnostic_log_)));
  }
  if (ledger_database_) {
...
void RewardsServiceImpl::OnStopLedgerForCompleteReset(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&DeleteFilesOnFileTaskRunner, diagnostic_log_.AsWeakPtr(),
                     paths),
...
void RewardsServiceImpl::DiagnosticLog(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&WriteToDiagnosticLogOnFileTaskRunner,
                     diagnostic_log_.AsWeakPtr(), diagnostic_log_path_,
                     log_entry, kTailDiagnosticLogToNumLines),
...
void RewardsServiceImpl::LoadDiagnosticLog(
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&LoadDiagnosticLogOnFileTaskRunner,
                     diagnostic_log_.AsWeakPtr(), num_lines),
      base::BindOnce(&RewardsServiceImpl::OnDiagnosticLogLoaded, AsWeakPtr(),
                     std::move(callback)));
}
